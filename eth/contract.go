...
func DeployContract(url, privateKey, nonce, gasLimit, gasPrice, jsonAbi, code string, args ...interface{}) (string, string, error) {
	...
	paramBytes, err := abiObj.Pack("", args...)
	if err != nil {
		return "", "", err
	}
	dataBytes := append(codeBytes, paramBytes...)
	...
	txObj := NewTxObj(nonce, "", "", gasLimit, gasPrice, "0x"+common.Bytes2Hex(dataBytes))
